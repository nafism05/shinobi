...
module.exports=function(__dirname,config){
	...
	s.detectObject=function(buffer,d,tx,frameLocation){
		plugLog("AWLog-pluginBase - detectObject does not exist");
	}
	// --- ADDED CODE ---
	s.awPythonEventController=function(d,cn,tx){
		plugLog("AWLog-pluginBase - awPythonEventController does not exist");
	}
	// --- ADDED CODE ---
	s.onPluginEvent = [];
		...
		switch(d.f){
			...
			case'init_monitor':
				...
				if(s.group[d.ke]&&s.group[d.ke][d.id]){
					...
					if(config.plug==="AwPython") s.awPythonEventController(d,cn,tx); // --- ADDED CODE ---
				}
			break;
			case'frameFromRam':
				if(!s.group[d.ke]){
	...
	if(config.mode==='host'){
		...
		s.cx=function(x){
			...
			//plugLog("AWLog-pluginBase - data sent from pluginBase to host (socketio.js): d.f = " + sendData.f);
		...
		io.on('connect',function(d){
			plugLog("AWLog-pluginBase - connect");
