...
$user.ws.on('f',function (d){
    ...
    switch(d.f){
        ...
        break;
        // --- ADDED CODE ---
        case'detector_awpmodule_list':
            d.tmp='';
            $.each(d.awpmodule,function(n,v){
                d.tmp+='<li class="mdl-list__item">';
                d.tmp+='<span class="mdl-list__item-primary-content">';
                d.tmp+=v;
                d.tmp+='</span>';
                d.tmp+='<span class="mdl-list__item-secondary-action">';
                d.tmp+='<label class="mdl-switch mdl-js-switch mdl-js-ripple-effect">';
                d.tmp+='<input type="checkbox" value="'+v+'" detailContainer="detector_awpmodule" detailObject="'+v+'" class="detector_awpmodule_selection mdl-switch__input"/>'; // detailContainer="detector_awpmodule" FOR WHAT?
                d.tmp+='</label>';
                d.tmp+='</span>';
                d.tmp+='</li>';
            });
            $('#detector_awpmodule_list').html(d.tmp);
            componentHandler.upgradeAllRegistered();
            //add auto select for preferences
            d.currentlyEditing=$.aM.e.attr('mid');
            if(d.currentlyEditing&&d.currentlyEditing!==''){
                d.currentlyEditing=JSON.parse(JSON.parse($.ccio.mon[d.currentlyEditing].details).detector_awpmodule);
                $.each(d.currentlyEditing,function(m,b){
                    d.e=$('.detector_awpmodule_selection[value="'+m+'"]').prop('checked',true);
                    d.p=d.e.parents('.mdl-js-switch');
                    if(d.p.length>0){
                        d.p.addClass('is-checked');
                    }
